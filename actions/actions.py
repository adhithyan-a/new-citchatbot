...
class ActionTellName(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        current_name = next(tracker.get_latest_entity_values("nameof"), None)
        utc = arrow.utcnow()
        
        if not current_name:
            msg = "I didn't get you name. Are you sure it's spelled correctly?"
            dispatcher.utter_message(text=msg)
            return []
        
        nz_string = name_db.get(current_name, None)
        if not nz_string:
            msg = f"I didn't recognize {current_name}. Is it spelled correctly?"
            dispatcher.utter_message(text=msg)
            return []
        
        msg = f"Sure thing! I'll remember that your name is {current_name}."
        dispatcher.utter_message(text=msg)
        
        return [SlotSet("user", current_name)]
